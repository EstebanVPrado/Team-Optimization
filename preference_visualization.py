...
def set_color_by_team(challenge):
    if challenge == 0:
...
def draw_coordinate_system():
    ...
    draw_arrow(0, 0, border, 0)
    draw_arrow(0, 0, 0, border)
    draw_arrow(0, 0, -border, 0)
    draw_arrow(0, 0, 0, -border)
    ...
    set_color_by_team(0)
    draw_text(-border - x_offset, border, "E.ON")
    set_color_by_team(1)
    draw_text(border + x_offset, border, "Merk")
    set_color_by_team(2)
    draw_text(-border - x_offset, -border, "BMW")
    set_color_by_team(3)
    draw_text(border + x_offset, -border, "Main Incubator")


def draw_participant(participant, color='by_role'):
    if color == 'by_team':
        set_color_by_team(participant.team)
    else:
...
if __name__ == "__main__":
    ...
    for c, participant in enumerate(participants):
        ...
        draw_participant(participant, 'by_team')
        for friend_name in participant.friends:
            if isinstance(friend_name, str):
                ...
                # set_color(participant.role)
                set_color_by_team(participant.team)
                draw_arrow(participant.x, participant.y, friend.x, friend.y)
